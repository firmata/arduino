...
void webSocketEvent(uint8_t num, WStype_t type, uint8_t * payload, size_t length) {
  ...
  switch(type) {
    ...
    case WStype_CONNECTED:
      {
        DEBUG_PRINT("Connected from ");
        DEBUG_PRINT(webSocket.remoteIP(num));
        DEBUG_PRINT(" url:");
        DEBUG_PRINTLN((const char*)payload);
        webSocket.sendTXT(num, "Firmata Server for your wireles service");
        clients++;
      }
      break;
    
    case WStype_TEXT:
      DEBUG_PRINT("Informative Text from websocket: ");
      DEBUG_PRINTLN((const char*)payload);

      // send message to client
      //webSocket.sendTXT(num, "hello from firmata");
      // send data to all connected clients
      // webSocket.broadcastTXT("hello to all firmata listeners");
      break;

    case WStype_BIN:
      DEBUG_PRINT("get Binary, length: ");
      DEBUG_PRINTLN(length);
#if 1
      // !! there is a Print::write(ptr,size_t) !?
      DEBUG_PRINT("before: ");
      DEBUG_PRINTLN(StreamFirmataInternal.available());
      for (size_t i = 0; i < length; i++)
      {
        Serial.printf("0x%02x ", (int)payload[i]);
        if (StreamAsFirmata.write(payload[i]) != 1) {
          DEBUG_PRINTLN("stream error writing to StreamAsFirmata");
        }
      }
      DEBUG_PRINTLN();
      DEBUG_PRINT("after: ");
    ...
    default:
      DEBUG_PRINTLN("TODO: unhandled WS message type");
  }
}
...
void loop()
{
  ...
  {
    last += 1000;
    DEBUG_PRINT("Firmata to WS: available=");
    DEBUG_PRINT(StreamAsFirmataLen);
    DEBUG_PRINT(" --- WS to Firmata: available=");
    DEBUG_PRINTLN(StreamFirmataInternal.available());
  }

  if (clients && StreamAsFirmataLen) {
    DEBUG_PRINT("Firmata -> WS: ");
    DEBUG_PRINT(StreamAsFirmataLen);
    DEBUG_PRINT(" bytes");
    ...
    {
      DEBUG_PRINT("Successfully broadcasted-to-websocket a binary message of ");
    }
    else
    {
      DEBUG_PRINT("Error broadcasting-to-websocket a binary message of ");
    }
    DEBUG_PRINT(StreamAsFirmataLen);
    DEBUG_PRINTLN(" bytes");
  }
